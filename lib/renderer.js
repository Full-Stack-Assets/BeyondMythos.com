...
function renderMonetization(site, root = "") {
  ...
  const sponsor = commerce.sponsor
    ? `<div class="sponsor"><span class="mono-label">${escapeHtml(commerce.sponsor.label)}</span><p><strong>${escapeHtml(commerce.sponsor.name)}</strong> — ${escapeHtml(commerce.sponsor.tagline)}</p><a href="${escapeHtml(commerce.sponsor.url)}" rel="sponsored noopener nofollow">Visit sponsor →</a></div>`
    : "";
  const products = commerce.products
    .map(
      (product) => `<article class="commerce-item"><strong>${escapeHtml(product.name)}</strong><p>${escapeHtml(product.description)}</p><a href="${escapeHtml(storeBase)}/store#product-${product.id}">View product →</a></article>`
    )
    .join("\n");
  const marketplaces = commerce.marketplaces.length
    ? `<div class="market-links">${commerce.marketplaces.map((link) => `<a href="${escapeHtml(link.url)}" rel="noopener nofollow">${escapeHtml(link.label)}</a>`).join("\n")}</div>`
    : "";
