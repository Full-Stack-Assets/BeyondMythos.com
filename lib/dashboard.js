...
function renderDashboard(sites, baseUrl, portfolio = null) {
  ...
  const digitalProducts = listProducts({ type: "digital" }).slice(0, 6);
  const commercePanel = `<section class="commerce-panel">
      <div>
        <p class="eyebrow">Monetization</p>
        <h2>Turn the network into a product funnel</h2>
        <p>Promote affiliate offers, sponsor slots, digital downloads, and external marketplace storefronts from every generated site.</p>
      </div>
      ${sponsor ? `<div class="mini-card"><strong>${escapeHtml(sponsor.name)}</strong><span>${escapeHtml(sponsor.tagline)}</span><a href="${escapeHtml(sponsor.url)}" rel="sponsored nofollow noopener">Sponsor link →</a></div>` : ""}
      <div class="commerce-grid">${digitalProducts.map((product) => `<div class="mini-card"><strong>${escapeHtml(product.name)}</strong><span>${escapeHtml(product.description)}</span><a href="/store#product-${product.id}">View digital catalog →</a></div>`).join("")}</div>
      <div class="market-links">${marketplaces.map((link) => `<a href="${escapeHtml(link.url)}" rel="noopener nofollow">${escapeHtml(link.label)}</a>`).join("")}</div>
      <p class="disclosure">${escapeHtml(affiliateDisclosure())}</p>
    </section>`;
  const portfolioPanel = portfolio
    ? `<section class="commerce-panel">
      <div>
        <p class="eyebrow">Portfolio operating model</p>
        <h2>Tiered growth, monetization, and domain governance</h2>
        <p>Track domain roles, north-star KPIs, and a shared 90-day campaign cadence across the network.</p>
      </div>
      <div class="commerce-grid">
        <div class="mini-card"><strong>${portfolio.domainCount}</strong><span>mapped domains</span></div>
        <div class="mini-card"><strong>${portfolio.tierCount}</strong><span>active strategy tiers</span></div>
        <div class="mini-card"><strong>${portfolio.weeklyThemes}</strong><span>weekly campaign themes</span></div>
      </div>
      <p class="disclosure">Revenue checkpoints include checkout conversion, refunds, and top funnels in <code>/api/portfolio/dashboard</code>.</p>
    </section>`
    : "";
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .commerce-panel {
      margin: 0 0 1.5rem; padding: 1.35rem; border: 1px solid var(--border); border-radius: 1rem;
      background: linear-gradient(135deg, rgba(96,165,250,0.1), rgba(249,115,22,0.08));
    }
    .commerce-panel h2 { margin: 0.2rem 0 0.4rem; }
    .commerce-grid { display: grid; gap: 0.8rem; grid-template-columns: repeat(auto-fit, minmax(210px, 1fr)); margin-top: 1rem; }
    .mini-card { border: 1px solid var(--border); border-radius: 0.8rem; padding: 0.9rem; background: rgba(255,255,255,0.04); }
    .mini-card strong, .mini-card span { display: block; }
    .mini-card span { color: var(--muted); font-size: 0.88rem; margin: 0.35rem 0; }
    .mini-card a, .market-links a { color: var(--accent); text-decoration: none; font-weight: 700; }
    .market-links { display: flex; gap: 0.75rem; flex-wrap: wrap; margin-top: 1rem; }
    .disclosure { color: var(--muted); font-size: 0.78rem; }
